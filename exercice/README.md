...
Les courbes de Bézier sont des courbes qui se définissent à l'aide d'un point de départ et d'un point d'arrivée additionnés de points intermédiaires (dits « points d'inflexion ») qui vont déformer le tracé et produire une courbe.

En théorie, il est possible de créer autant de points d'inflexion qu'on le souhaite. Cependant, en SVG il n'existe que deux types de courbes de Bézier :
...
- _les courbes « cubic »_ (avec deux points d'inflexion).<br>
  ...
  > **ASTUCE :** Sur toutes les commandes SVG, le dernier jeu de coordonnées `x,y` représente toujours le point d'arrivée du segment de courbe défini par la commande.
...
Les arcs de cercle permettent de définir des courbes qui sont des segments de cercle. C'est parfois plus simple à utiliser que des courbes de Bézier, qui sont assez difficiles à manipuler quand on veut rendre des formes très régulières comme des cercles ou des ellipses.

  > **INFO :** On parle d'arc de cercle par abus de langage mais en fait il serait plus juste de parler d'arc d'ellipse car le cercle n'est qu'un cas particulier de l'ellipse.
...
- La paire `X,Y` représente les coordonnées du point d'arrivée de l'arc de cercle.
...
Pour en savoir plus : [Elliptical Arc Curve on MDN][3]


## Objectif de l'exercice

Dans un document SVG avec une `viewBox` de 100x100, dessiner un cœur centré dans la `viewBox` en utilisant deux courbes de Bézier _quadratic_ (pour les côtés) et deux arcs de cercle (pour les deux ventricules en haut du cœur).
...
[3]: https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/d#Elliptical_Arc_Curve
